...
// Sizing variables for our chart. These are saved as variables as they will be used in calculation.
var chartWidth = 300;
var chartHeight = 100;
var padding = 5;

// We want our bars to take up the full height of the chart. so we will apply a scaling factor
// to the height of every bar.
var heightScalingFactor = chartHeight / getMax(dataset);

// Here we are creating the SVG that will be our chart.
var svg = d3.select('#my-chart') 			// I'm starting off by selecting the container.
...
svg.selectAll('rect')										// I'm selecting all of the rectangles in the SVG (note that at this point, there actually aren't any, but we'll be creating them in a couple of steps).
   .data(dataset)											// Then I'm mapping the dataset to those rectangles.
   .enter()													// This step is important in that it allows us to dynamically create the ractangle elements that we selected previously.
   ...
   .attr('y', function (value, index) { 					// Set the rectangle by subtracting the scaled height from the height of the chart (this has to be done because SVG coordinates start with 0, 0 at their top left corner).
